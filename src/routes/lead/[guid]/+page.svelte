...
<div class="gridevnheader">
	<div class="evnheader">
		<img style="height: 40px" src="images/eVnBestellungCWLogo.png" alt="evn-logo" />
	</div>
	<div class="evnheader">{texte.header[rawLead.sprache]}</div>
</div>
{#if data.guid}
	{#if !showMail}
		{#if !rawLead.evnsent}
			<Evn {rawLead} lang={rawLead.sprache}></Evn>
		{:else}
			<EvnStatus bind:showMail status={rawLead.evnstatus}></EvnStatus>
		{/if}

		<div class="gridevnfooter">
			<div class="r12 btop"></div>
			...
			<div>
				<img style="height: 60px" src="images/SWLogoCWeVn.png" alt="evn-logo" />
			</div>
		</div>
