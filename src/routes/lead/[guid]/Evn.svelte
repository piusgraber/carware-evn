<script>
	...
	let disponenten = $state(rawLead.disponenten);
...
//	let evnValid = false;
	...
	$effect(() => {
		...
		if (rawLead.garage_idkey == 'AUTOKUNZ') {
            ...
            evnValid = checkFormsValid() && halterValid && (!needsStandort || (needsStandort && standortValid));
        }
	});
	$effect(() => {
...
<div class="evn">
	<div class="r14 headline">{texte.headline1[rawLead.sprache]}</div>
<div class="r14 headline">{texte.headline2[rawLead.sprache]}</div>
<div>&#160;</div>
	<div class="r14">
	...
	<div class="c12 top">{texte.bemerkung[lang]}</div>
	<div class="c12">
...
<div class="evn2">
	...
	<div class="r4c2 bg2">
		...
		{texte.datenschutz2[lang]}<br /><br /> 
		<u>{texte.gueltigkeit[lang]}</u>
		{@html texte.gueltigkeit2[lang]}<br /><br /> 
		
		<input type="checkbox" bind:checked={dataChecked} />
		<span class="link" on:click={toggleHalterCheck}>
			...
			<span>{texte.pruefung4[lang]}</span>
			<span class:invalid={!dataChecked}>{texte.pruefung5[lang]}</span>
			<span>{texte.pruefung6[lang]}</span>
		</span>
		<span>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;</span>
	</div>
	<div class="r13"></div>
	...
	<div>
		{#if false} d{dataChecked} f{fzValid} h{halterValid} s{!needsStandort || standortValid} e{evnValid}{/if}</div>
	<div class="r33">
		<button class="submit"
			...
			disabled={btnClicked ||
				...
				!fzValid ||
				!halterValid || 
				needsStandort && !standortValid }>{texte.bestellen[lang]}</button
		>
		{#if sent}
			{#if sendMessage}
...
<style>
	...
	.evn {
		display: grid;
		grid-template-columns: 220px 420px 220px 490px;
		align-items: center;
	...
	.evn2 {
		display: grid;
		grid-template-columns: 220px 320px 320px 400px 100px;
	}
