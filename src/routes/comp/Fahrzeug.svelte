<script lang="ts">
	...
	let { fahrzeug = $bindable({test:0}), lang, fzValid = $bindable(), demo } = $props();

import {lead} from "$lib/lead.svelte"

	import { texte } from '$lib/texte.js';
	import { dateDisp, options } from '$lib/time.js';
...
//    const counter = createCounter()
    ...
    let previousFahrzeug
    let oldChassis = fahrzeug.chassisnr
    $effect(() => {
        console.log('Chassis changed:', fahrzeug.chassisnr);
        if (fahrzeug.chassisnr != oldChassis) {
            console.log('Chassis changed:', fahrzeug.chassisnr);
            }
  });
	...
	$effect(() => {
		if (fahrzeug.chassisnr) {
            console.log(fahrzeug.chassisnr)
			invalidchassisnr = !chassisnrValid(fahrzeug.chassisnr);
            console.log('chassis', invalidchassisnr)
		}
	});
	...
	$effect(() => {
        if (fahrzeug.stammnr) {
            const newInvalidStammnr = !stammnrValid();
            if (invalidstammnr !== newInvalidStammnr) {  // Only update if the value changes
                invalidstammnr = newInvalidStammnr;
                console.log('isn', fahrzeug.stammnr, invalidstammnr);
            }
        }
	});
	// Kennzeichen
...
*/
	...
	/*    
    $: invalidAufbau =
        fahrzeug && (!fahrzeug.aufbau || fahrzeug.aufbau.length < 3);
*/
	const npValid = () => {
...
*/
	...
	let verkaeuferAxaAgent = { choice: 0 };
...
<h1>Fahrzeug</h1>
<hr/>
data: {JSON.stringify(fahrzeug)}
<hr/>
<!--
-->
NEU{ texte.neu[lang].toLowerCase()}
