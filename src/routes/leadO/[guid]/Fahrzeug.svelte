            ...
            for (let i=0; i<fahrzeug.kennzeichen.length; i++) {
                ...
                if (fahrzeug.kennzeichen[i] < '0' || fahrzeug.kennzeichen[i] > '9') {
                    if (i != last) {
                        ...
                        if (fahrzeug.kennzeichen[i].toUpperCase() != 'U' && fahrzeug.kennzeichen[i].toUpperCase() != 'Z' && fahrzeug.kennzeichen[i] != ' ') {
                            return false;
                        }
                    }
                }
...
/*
        ...
        if (fahrzeug.invsetz) {
            vnab = new Date(fahrzeug.vnab);
            invs = new Date(fahrzeug.invsetz);
            deadline = invs.addDays(-1);
        }
        if (!short && vnab < deadline) {
            return false;
        }
*/
        return true;
...
//            alert(err); // Failed to fetch
    ...
    $effect(() => {
        ...
        if (fahrzeug) {
            ...
            if (fahrzeug.versicherung == "AXA") {
                ...
                let em = versicherungen.filter(
                    (v) => v.id == fahrzeug.versicherung
                );
                fahrzeug.versicherungemail = "";
                if (em.length) {
    ...
    let axaAgenturChoice = 1;
    let verkaeuferAxaAgent = $state({choice: 0});
    let verkaeuferTempAxaAgent = {};
 ...
 --btn-bg-color:${$styles.btnBgColor};
 --btn-disabled-bg-color:${$styles.btnDisabledBgColor};
...
<div class="gridevn2">
    ...
    <div>
        {#if short}
            <button class="small" on:click={toggleSL}>{texte.alle[lang]}</button
            >
        {:else}
<!--
...
-->
    ...
    {#if !short}
        ...
        <div>
            ...
            type="date" bind:value={fahrzeug.invsetz} required="true" /> 
        </div>
...
-->
        ...
        <div>
            <input class:invalid={invalidPreis} type="number" bind:value={fahrzeug.katalogpreis} required="true" /> CHF
        </div>
        <div>{texte.zub[lang]}</div>
        <div>
            <input class:invalid={invalidPreisZB} type="number" bind:value={fahrzeug.preiszubehoer} required="true" /> CHF 
        </div>
        <div></div>
        <div>
        </div>
        ...
        <div>
            <input class:invalid={invalidPreisZB} type="number" bind:value={fahrzeug.preiszusatzzubehoer} required="true" /> CHF 
        </div>
    {:else}
        <div class="span4">
            <hr />
    ...
    <div>
        ...
        bind:value={fahrzeug.kennzeichen} required="true" />
        <span on:click={setNew} style="cursor: pointer"
            >[{texte.neu[lang]}]</span
        >
    </div>
    ...
    <div>
        <select
            title={fahrzeug.versicherungemail}
            bind:value={fahrzeug.versicherung}
            required="true"
        >
    ...
    {#if fahrzeug.versicherung == "AXA"}
        ...
        <div class="r12">
            <AxaAgentur
            bind:verkaeuferAxaAgent={verkaeuferAxaAgent}
                sprache={lang}
            />
        </div>
    ...
    <div>
        <input
            class:invalid={invalidvnab}
            type="date"
            bind:value={fahrzeug.vnab}
            required="true"
        />
    </div>
    ...
    <div>
        <input type="text" bind:value={fahrzeug.typenschein} required="true" />
    </div>
...
<style>
    ...
    .gridevn2 {
        display: grid;
        grid-template-columns: 220px 400px 220px 470px;
        grid-auto-rows: auto;
        grid-column-gap: 0px;
        grid-row-gap: 0px;
    }
